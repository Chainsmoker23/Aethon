          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            <div className="flex bg-slate-100 dark:bg-zinc-900 p-1 rounded-xl shadow-inner border border-slate-200 dark:border-zinc-800">
              <button 
                ...
                className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-lg transition-all ${filter === "all" ? "bg-white dark:bg-[#0a0a0a] text-slate-900 dark:text-zinc-100 dark:text-zinc-900 shadow-sm" : "text-slate-500 dark:text-zinc-500 dark:text-slate-500 dark:text-zinc-500 hover:text-slate-900 dark:hover:text-white dark:text-zinc-100 dark:text-zinc-900"}`}
              >
                All
              </button>
              <button 
                onClick={() => setFilter("open")}
                className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-lg transition-all ${filter === "open" ? "bg-white dark:bg-[#0a0a0a] text-rose-600 shadow-sm" : "text-slate-500 dark:text-zinc-500 dark:text-slate-500 dark:text-zinc-500 hover:text-rose-600"}`}
              >
                Open
              </button>
              <button 
                onClick={() => setFilter("resolved")}
                className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-lg transition-all ${filter === "resolved" ? "bg-white dark:bg-[#0a0a0a] text-emerald-600 shadow-sm" : "text-slate-500 dark:text-zinc-500 dark:text-slate-500 dark:text-zinc-500 hover:text-emerald-600"}`}
              >
                Resolved
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-[#0a0a0a] border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden animate-fade-in-up delay-100 divide-y divide-slate-100 shadow-sm">
            {filtered.length === 0 ? (
              <div className="p-16 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-zinc-100 dark:text-zinc-900">No escalations found</h3>
                <p className="text-sm font-medium text-slate-500 dark:text-zinc-500 dark:text-slate-500 dark:text-zinc-500 mt-2">Try adjusting your search or filters.</p>
              </div>
            ) : (
              filtered.map((e, i) => {
                ...
                return (
                  <div key={e.id} className="p-6 md:p-8 hover:bg-slate-50 dark:hover:bg-zinc-800/50 dark:bg-zinc-900/50 dark:hover:bg-slate-800 dark:hover:bg-zinc-300/50 dark:bg-zinc-900/50 transition-colors flex flex-col md:flex-row gap-6 md:items-start group">
                    ...
                    <div className="shrink-0 pt-1">
                      {e.is_resolved ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-rose-500 text-white dark:text-zinc-900 flex items-center justify-center shadow-lg shadow-rose-500/40 pulse-dot">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-3 mb-2">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-zinc-100 dark:text-zinc-900 truncate">
                          {e.residents?.first_name} {e.residents?.last_name}
                        </h3>
                        <span className="text-xs font-bold text-slate-600 dark:text-zinc-400 bg-slate-100 dark:bg-zinc-900 px-2 py-1 rounded-md border border-slate-200 dark:border-zinc-800 shadow-sm">
                          Room {e.residents?.room_number || 'N/A'}
                        </span>
                      </div>
                      <p className="text-sm text-slate-600 dark:text-zinc-400 font-medium leading-relaxed max-w-3xl">
                        {e.reason}
                      </p>
                    </div>
                    ...
                    <div className="shrink-0 flex flex-row md:flex-col items-center md:items-end gap-4 md:gap-2 text-sm border-t border-slate-100 dark:border-zinc-800/50 md:border-t-0 pt-4 md:pt-0">
                      <div className="flex items-center gap-2 text-slate-500 dark:text-zinc-500 dark:text-slate-500 dark:text-zinc-500 font-medium">
