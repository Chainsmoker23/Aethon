...
export function MobileResidentRoster({ residents, expandedId, setExpandedId }: { residents: any[], expandedId: string | null, setExpandedId: (id: string | null) => void }) {
  return (
    <div className="divide-y divide-slate-100 bg-white dark:bg-slate-900 md:hidden">
      {residents.map((r) => {
        const isExpanded = expandedId === r.id;
        return (
          <div key={r.id} className="group">
            <div
              className={`flex flex-col gap-1.5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50 transition-colors p-3 ${r.escalations > 0 ? "bg-red-50/30 border-l-[4px] border-red-500" : "border-l-[4px] border-transparent"}`}
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                    r.careStage === "Independent" ? "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400" :
            ...
            {/* Expanded Area */}
            {isExpanded && (
              <div className="bg-slate-50 dark:bg-slate-800/50/80 px-4 py-4 border-t border-slate-100 dark:border-slate-800/50 shadow-inner">
                ...
                {r.recentVisits.length === 0 ? (
                  <p className="text-xs text-slate-500 dark:text-slate-500 italic bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-800">No recent visits logged.</p>
                ) : (
                  <div className="space-y-2">
                    {r.recentVisits.map((v: any, i: number) => (
                      <div key={i} className="flex gap-3 bg-white dark:bg-slate-900 p-2.5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-0.5">
                            <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">{v.type}</span>
