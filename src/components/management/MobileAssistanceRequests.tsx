  ...
  return (
    <div className="bg-white dark:bg-[#0a0a0a] rounded-xl shadow-sm border border-slate-200 dark:border-zinc-800 overflow-hidden relative z-10 md:hidden">
      <div className="px-4 py-3 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between bg-slate-50 dark:bg-zinc-900/50">
        <h2 className="text-sm font-semibold text-slate-900 dark:text-zinc-100 dark:text-zinc-900 flex items-center gap-2 tracking-tight">
      ...
      <div className="divide-y divide-slate-100">
        {requests.length === 0 ? (
          <div className="p-6 text-center text-slate-500 dark:text-zinc-500 dark:text-slate-500 dark:text-zinc-500 text-xs">
            <CheckCircle2 className="w-8 h-8 text-emerald-400 mx-auto mb-2 opacity-50" />
            <p>No active escalations</p>
          </div>
        ) : (
          requests.map((req) => (
            <div key={req.id} className="p-3 hover:bg-slate-50 dark:hover:bg-zinc-800/50 dark:bg-zinc-900/50 dark:hover:bg-slate-800 dark:hover:bg-zinc-300/50 dark:bg-zinc-900/50 transition-colors flex flex-col gap-2">
              <div className="flex justify-between items-start gap-2">
                <div className="flex flex-col items-start min-w-0">
                  <p className="font-semibold text-slate-900 dark:text-zinc-100 dark:text-zinc-900 text-xs line-clamp-1">{req.resident_name}</p>
              ...
              <p className="text-[11px] text-slate-600 dark:text-zinc-400 line-clamp-2 leading-relaxed">{req.description}</p>
            </div>
          ))
        )}
      </div>
