    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative z-10 md:hidden">
      ...
      <div className="divide-y divide-slate-100">
        {requests.length === 0 ? (
          ...
          requests.map((req) => (
            <div key={req.id} className="p-3 hover:bg-slate-50 transition-colors flex flex-col gap-2">
              <div className="flex justify-between items-start gap-2">
                <div className="flex flex-col items-start min-w-0">
                  <p className="font-semibold text-slate-900 text-xs line-clamp-1">{req.resident_name}</p>
                  <p className="text-[11px] font-medium text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded mt-1">{req.issue_type}</p>
                </div>
                <div className="flex items-center gap-1 text-[10px] font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded shrink-0">
