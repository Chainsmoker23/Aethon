    ...
    <>
      ...
      <div className="h-14 lg:h-[88px] flex items-center justify-between px-4 lg:px-8 border-b border-slate-200 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a] shrink-0">
        <div className="hidden lg:flex items-center gap-3">
          ...
          />
          <span className="font-bold text-slate-900 dark:text-zinc-100 dark:text-zinc-900 tracking-tight">Management</span>
        </div>
        {/* Mobile close button */}
        <button
          ...
          return (
            <Link 
              ...
              className={`flex items-center gap-3 px-3 py-3 rounded-xl font-bold text-sm transition-all ${
                isActive 
                  ? "bg-gradient-to-r from-cyan-500 to-indigo-500 text-white dark:text-zinc-900 shadow-md shadow-indigo-500/20" 
                  : "text-slate-600 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-800 dark:bg-zinc-900 dark:hover:bg-slate-800 dark:hover:bg-zinc-300 dark:bg-zinc-900 hover:text-slate-900 dark:hover:text-white dark:text-zinc-100 dark:text-zinc-900"
            ...
            >
              ...
              {link.badge && link.badge > 0 ? (
                <span className={`text-[10px] font-black px-2 py-0.5 rounded-full min-w-[24px] text-center ${
                  isActive ? "bg-white dark:bg-[#0a0a0a]/20 text-white dark:text-zinc-900" : "bg-rose-500 text-white dark:text-zinc-900 shadow-sm"
                }`}>
                  {link.badge}
                </span>
              ) : null}
      ...
      <div className="p-4 border-t border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-900/50 m-4 rounded-2xl shrink-0">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white dark:text-zinc-900 font-black text-sm shadow-sm">
            {initials}
          </div>
          <div className="min-w-0">
            <p className="font-extrabold text-sm text-slate-900 dark:text-zinc-100 dark:text-zinc-900 truncate">{userName}</p>
            <p className="text-slate-500 dark:text-zinc-500 dark:text-slate-500 dark:text-zinc-500 text-[10px] uppercase tracking-wider font-bold mt-0.5">Management</p>
          </div>
        </div>
          ...
          return (
            <Link 
              ...
              className={`relative flex flex-col items-center justify-center w-full py-3 gap-1 transition-colors ${
                isActive ? "text-indigo-600" : "text-slate-400 hover:text-slate-600 dark:text-zinc-400"
              }`}
            >
              <div className="relative">
                {link.icon}
                {link.badge && link.badge > 0 ? (
                  <span className="absolute -top-1.5 -right-2 bg-rose-500 text-white dark:text-zinc-900 text-[8px] font-black px-1.5 py-0.5 rounded-full shadow-sm">
                    {link.badge}
                  </span>
                ) : null}
              </div>
              <span className={`text-[10px] font-bold ${isActive ? "text-slate-900 dark:text-zinc-100 dark:text-zinc-900" : "text-slate-500 dark:text-zinc-500 dark:text-slate-500 dark:text-zinc-500"}`}>
                {link.name}
              </span>
            </Link>
          );
